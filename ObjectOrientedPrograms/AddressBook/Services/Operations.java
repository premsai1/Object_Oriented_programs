 ...
 * @purpose : this the performs all the implementations of operations like add
 *          ,delete,print,delete operations.
 * 
 */
public class Operations {
	...
	public void PrintUser(List<PersonInfo> listOfPerson) {
		...
		for (int i = 0; i < listOfPerson.size(); i++) {
			if (listOfPerson.get(i).getFname().equals(name)) {
				...
				System.out.println(
						temp.getFname() + "  " + temp.getLname() + "  " + temp.getMbno() + "  " + temp.getAddress()
	...
	public List<PersonInfo> SortUser(List<PersonInfo> listOfPerson) {
		List<PersonInfo> sortedList = listOfPerson.stream().sorted(Comparator.comparing(PersonInfo::getFname))
